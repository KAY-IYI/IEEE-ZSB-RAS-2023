...
int main()
{
	...
	GI_enu_Enable(); // Enable global interrupts
	LCD_enu_WriteString("Please push");
	LCD_enu_GoToXY(1, 0);
	LCD_enu_WriteString("The button");

	// Configure EXTI to trigger the login process when the user presses a button
	EXTI_enu_Enable(EXTI_INT0, EXTI_ANY_CHANGE);
	EXTI_enu_SetCallback(EXTI_INT0, &StartLoginProcess);

	while (1)
		; // Wait for the EXTI interrupt to trigger the login process
...
void StartLoginProcess(void)
{
	GI_enu_Disable(); // Disable global interrupts during password entry
			...
			if (Local_u8_key >= '0' && Local_u8_key <= '9')
			{
				...
				if (Local_u8_passwordIndex == PASSWORD_LENGTH)
				{
					...
					if (Local_u8_isPasswordCorrect)
					{
						...
						LCD_enu_GoToXY(1, 0);
						LCD_enu_WriteString("3:Servo  4:EXIT");
						_delay_ms(100);
						u8 Local_u8_key2;
	...
	GI_enu_Enable(); // Re-enable global interrupts before exiting the login process
}
...
void takeUserOption_1(u8 *option)
{
	while (1)
	{
		...
		if (*option == '1' || *option == '2' || *option == '3' || *option == '4' || *option == 'C')
			break;
	}
}
...
{
	LCD_enu_Clear();
	DIO_enu_SetPinDirection(Servo_u8_PORT,Servo_u8_pin,DIO_u8_OUTPUT);
	LCD_enu_WriteString("Angel: in (3DIG) ");
	LCD_enu_GoToXY(1, 0);
	LCD_enu_WriteString("EX: 090 ");
	u8 Local_u8_Angle = KPD_enu_GetNumberInput();
	...
	while (1)
	{
		...
		u8 motorValue = (digitalValue * 1000UL) / 1023;
		Servo_enu_SetServoAngle(motorValue);
	}
}
