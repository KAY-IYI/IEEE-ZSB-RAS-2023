/* For the previous problem, write a
function that accepts the student_info as
an input argument and prints the info of
the 15 student and returns number of
...
void Student_Func(struct Student_Data Student[15])
{
    int count ;
    ...
    printf("Number students with marks less than 12 is :-   %d",count);
...
int main(){
    ...
    if (Student[i].mark > 20 || Student[i].mark < 0)
        {
            printf("Please Re-Enter the Student mark\n");
            printf("Enter %d Student Mark : ",Student[i].roll_number);
            scanf("%f",&Student[i].mark);
        }
